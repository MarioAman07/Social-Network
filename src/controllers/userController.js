...
module.exports = {
  ...
  registerUser: async (req, res) => {
    try {
      ...
      const newUser = {
        username,
        email,
        password_hash: hashedPassword, // Храним только хеш!
        bio: "",
  ...
  getUserStats: async (req, res) => {
    try {
      ...
      const pipeline = [
        { $match: { user_id: userId } }, // 1. Находим все посты юзера
        { 
          $group: { 
            _id: "$user_id", 
            totalPosts: { $sum: 1 }, 
      ...
      res.json({ user, stats: stats[0] || { totalPosts: 0, totalLikesReceived: 0 } });
    } catch (err) {
      res.status(500).json({ error: err.message });
    }
  }
